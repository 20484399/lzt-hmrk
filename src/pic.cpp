...
namespace ImgParse
{
	...
	vector<Point2f> FindConner(Mat& mat)
	{
		...
		int poi[4][2] = { {0,0},{1359,0},{0,1359},{1359,1359} };
		vector<Point2f> ret;
		for (int k = 0; k < 4; ++k)
		{
			bool ispass[16][16] = { 0 };
	...
	bool Main(const Mat& srcImg, Mat& disImg)
	{
		...
		//以下为定位过程
		{
			...
			//三阶微调，完成最终矫正
			disImg.copyTo(temp);
			cv::resize(temp, temp, Size(1360, 1360));
			GetVec(temp);
			auto poi4 = FindConner(temp);
			if (poi4.size() != 4) return 1;
			cv::resize(disImg, disImg, Size(1360, 1360));
			temp = CropParallelRect(disImg, poi4, Size(1360, 1360));
		}
